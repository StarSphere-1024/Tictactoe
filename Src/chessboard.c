...
const ChessboardFuc_t ChessboardFuc = {.init = init,
                                       .set_pieces = set_pieces,
                                       .is_win = is_win,
                                       .show = ShowChessBoard};

void init(Chessboard *self) { memset(self->data, ' ', 9); }

static bool set_pieces(Chessboard *self, Player player, Coord location) {
  if (location.x >= 3 || location.y >= 3) {
    printf("Invalid location! Must be between 0 and 2.\n");
    return false;
  }
...
static unsigned char is_win(Chessboard *self) {
  for (int i = 0; i < 3; i++) {
    if (self->data[i][0] == self->data[i][1] &&
        self->data[i][1] == self->data[i][2] && self->data[i][1] != NULL_FLAG) {
      return self->data[i][1];
    }
  }
  // 三列
  for (int i = 0; i < 3; i++) {
    if (self->data[0][i] == self->data[1][i] &&
        self->data[1][i] == self->data[2][i] && self->data[1][i] != NULL_FLAG) {
      return self->data[1][i];
    }
  }
  // 对角线相同
  if (self->data[0][0] == self->data[1][1] &&
      self->data[1][1] == self->data[2][2] && self->data[1][1] != NULL_FLAG) {
    return self->data[1][1];
  }
  if (self->data[0][2] == self->data[1][1] &&
      self->data[1][1] == self->data[2][0] && self->data[1][1] != NULL_FLAG) {
    return self->data[1][1];
  }
